...
필요시 별도 탐색 세션이나 병렬 파일 읽기를 사용해 관련 코드와 문서를 빠르게 확인한다.
...
사용자가 구현 계획 작성을 지시하면 여러 step으로 나뉜 초안을 작성해 피드백을 요청한다. 이 skill을 사용할 때는 계획 파일만 생성하고, 사용자가 별도로 지시하지 않는 한 실제 구현 코드는 작성하지 않는다.
...
- `dir`: task 디렉토리명.
- `status`: `"pending"` | `"completed"` | `"error"` | `"blocked"`. `scripts/execute.py`가 실행 중 자동으로 업데이트한다.
...
`scripts/execute.py`가 자동으로 처리하는 것:
